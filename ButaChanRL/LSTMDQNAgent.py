...
import numpy as np
from SequentialExperienceReplay import SequentialReplayBuffer
from Network import LSTMDQN
from Network import optimize_network_lstm
...
class LSTMDQNAgent:
    ...
    def agent_init(self, agent_config):
        ...
        self.replay_buffer = SequentialReplayBuffer(agent_config['replay_buffer_size'],
                                          agent_config['minibatch_sz'],self.seed)
        self.batch_size = agent_config['minibatch_sz']
    ...
    def greedy_policy(self,state,epsilon=0.001):
        ...
        if(a>=epsilon):
            with torch.no_grad():
                action_values,self.h_,self.c_ = self.q_network(state,self.h_,self.c_)
    ...
    def epsilon_greedy_policy(self,state):
        ...
        if(a>=epsilon):
            with torch.no_grad():
                action_values,self.h_,self.c_ = self.q_network(state.unsqueeze(1),self.h_.to(self.device),self.c_.to(self.device))
            action = torch.argmax(action_values).item()
        else:
            action = random.choice(list(range(self.num_actions)))
        return action
    ...
    def agent_start(self, state):
        ...
        """
        self.h_target,self.c_target=self.target_network.init_lstm(self.batch_size,training=False)
        self.h_,self.c_=self.q_network.init_lstm(self.batch_size,training=False)
        #print("h_ size",self.h_.shape)
        self.sum_rewards = 0
        self.episode_steps = 0
        self.last_state = torch.tensor(np.array([state]),dtype=torch.float32,device=self.device)
        #print("initial state",self.last_state.shape)
    ...
    def agent_step(self, reward, state):
        ...
        if self.replay_buffer.size() > self.replay_buffer.minibatch_size*2 and self.time_step>self.warmup_steps: # and self.episode_steps%self.replay_buffer.minibatch_size==0:
            for _ in range(self.num_replay):
                # Get sample experiences from the replay buffer
                experiences = self.replay_buffer.sample()
                #print("Step Experiences",experiences)
                loss = optimize_network_lstm(experiences, self.discount, self.optimizer, self.target_network, self.q_network,self.device,self.double_dqn)
    ...
    def agent_end(self, reward):
        ...
        self.replay_buffer.append(self.last_state, self.last_action, reward, terminal, state)
        # Perform replay steps:
        if self.replay_buffer.size() > self.replay_buffer.minibatch_size*2:
            for _ in range(self.num_replay):
                experiences = self.replay_buffer.sample()
                #print("End Experiences",experiences)
                loss = optimize_network_lstm(experiences, self.discount, self.optimizer, self.target_network, self.q_network,self.device,self.double_dqn)
                end_loss = loss
                if(len(self.loss)>=self.loss_capacity):
                    del self.loss[0]
